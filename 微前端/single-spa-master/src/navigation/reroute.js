 ...
 * 4类场景分别执行相应的操作，如更改app.status、执行生命周期函数
 * 从这里可以看出，single-spa就是一个维护应用的状态机
 * @export
...
export function reroute(pendingPromises = [], eventArguments) {
  ...
  function performAppChanges() {
    return Promise.resolve().then(() => {
      ...
      // 对于需要被卸载的app分两步处理 先卸载（执行unmount） 再移除（执行unload）
       ...
       */
      // 当卸载其他应用程序的时候加载和引导应用程序，等到所有应用程序都完成卸载之后再安装应用程序
      // 这个原因其实是因为这些操作都是通过注册不同的微任务实现的，而js是单线程执行，所以后续的自然只能等前面的执行完了才能执行
