  ...
  apps.forEach((app) => {
    ...
    switch (app.status) {
      ...
      case NOT_MOUNTED:
        ...
        break;
        // 需要被卸载的应用：已经处于挂载状态，但与当前路由不匹配的应用需要被卸载掉
      case MOUNTED:
        if (!appShouldBeActive) {
 ...
 * })
 * @export
 * @param {*} appNameOrConfig：应用名称
 * @param {*} appOrLoadApp：应用的加载function，是一个Promise
 * @param {*} activeWhen：判断当前应用是否激活，return boolean
...
export function registerApplication(
   ...
   * registration = {
   *    name: 'app1',
   *    loadApp: function return Promise,
   *    activeWhen: function return boolean,
   *    customProps: {},
   * }
   */
 ...
 * @export
 * @param {*} config = { name: 'app1', app: function (): Promise {}, activeWhen: function (): boolean {}, customProps: {} }
 */
export function validateRegisterWithConfig(config) {
  ...
  // app 属性只能是一个对象或者函数
  // 对象需包含单个spa的各个生命周期 函数则必须返回一个promise
  if (typeof config.app !== "object" && typeof config.app !== "function")
    ...
    // 第三个参数 可以是字符串或函数 也可以是两者组成的一个数组 表示当前应该被激活的应用的baseURL
...
function sanitizeArguments(
  ...
  // 如果第二个参数不是一个函数，包装成一个返回promise的函数
  registration.loadApp = sanitizeLoadApp(registration.loadApp);
  // 如果用户没有提供 props 对象，则给一个默认的空对象
  registration.customProps = sanitizeCustomProps(registration.customProps);
  // 确保activeWhen是一个返回boolean值的函数
...
// 得到一个函数 负责判断浏览器当前地址是否和用户给定的baseURL相匹配，匹配返回true，否则返回false
