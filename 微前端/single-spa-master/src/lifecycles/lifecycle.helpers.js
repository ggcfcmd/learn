...
/**
 * 返回一个接受props作为参数的函数，这个函数负责执行子应用中的生命周期函数，并确保生命周期函数返回的结果是promise
 * @param {*} appOrParcel appOrParcel => window.singleSpa 子应用打包后的对象
 * @param {*} lifecycle lifecycle => 字符串，生命周期名称
 * @returns
 */
export function flattenFnArray(appOrParcel, lifecycle) {
  let fns = appOrParcel[lifecycle] || [];
  fns = Array.isArray(fns) ? fns : [fns];
  // 有些生命周期函数子应用可能不会设置，比如unload，给个默认值
  if (fns.length === 0) {
    fns = [() => Promise.resolve()];
  }
